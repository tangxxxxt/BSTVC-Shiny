...
# 六、UI 前端
#
# 侧边栏展示完整工作流入口，并通过分割线区分数据处理、建模和关于信息。
# 数据转换、数据输入、数据检查和建模页面之间同时保留按钮式跳转，便于按流程操作。
...
ui <- bs4DashPage(
  ...
  sidebar = bs4DashSidebar(
    ...
    bs4SidebarMenu(
      ...
      tags$li(class = "nav-group-divider"),
      bs4SidebarMenuItem("BSTVC 建模", tabName = "bstvc", icon = icon("chart-line")),
      bs4SidebarMenuItem("BSVC 建模", tabName = "bsvc", icon = icon("chart-area")),
      tags$li(class = "nav-group-divider"),
    ...
    tags$head(
      tags$style(HTML(
        ...
        .nav-group-divider {
          height: 1px;
          margin: 12px 20px;
          background: linear-gradient(90deg, transparent, rgba(117,107,120,0.24), transparent);
          list-style: none;
        }
        ...
        .wechat-qr-img {
          ...
          border-radius: 8px;
          background: rgba(255,255,255,0.72);
          border: 1px solid rgba(72,58,69,0.10);
          padding: 8px;
        }
        .wechat-qr-placeholder {
          ...
          border: 1px dashed rgba(122,92,112,0.32);
          border-radius: 8px;
          text-align: center;
          font-size: 12px;
          line-height: 1.45;
        ...
        fluidRow(
            ...
            tagAppendAttributes(
              bs4UserCard(
                ...
                tags$div(
                  ...
                  tags$h2(class = "overview-user-title", "BSTVC-时空可解释性工具"),
                  tags$p(
                    class = "overview-user-text",
                    "BSTVC：在统一贝叶斯框架内同步实现局部时空可解释（刻画因素在具体时空单元的作用差异）、全局时空可解释（量化时空贡献、识别关键驱动因素）与时空动态预测（缺失值填补、未来趋势预测与时空平滑），一站式赋能地理时空研究与决策。"
                  ),
                  tags$div(
          ...
          column(
            ...
            bs4Card(
              ...
              tags$div(
                class = "info-block",
                tags$strong(icon("chart-line"), "BSTVC-时空可解释"),
                tags$p("面向连续时序的时空面板数据，探测解释变量与响应变量关系的时空非平稳性，解析因素作用的时空异质性机制与动态演变规律。")
              ),
              tags$div(
                class = "info-block",
                tags$strong(icon("chart-area"), "BSVC-空间可解释"),
          ...
          column(
            ...
            bs4Card(
              ...
              tags$ul(
                ...
                tags$li("如果原始建模表格是空间截面宽表类型，需要先进入“空间截面数据转换”界面，将其整理为时空面板格式，并下载数据。"),
                tags$li("如果已确认原始数据本身就是可建模的时空面板数据，或当前任务是 BSVC 空间截面建模，可以直接进入“数据输入”界面。")
              ),
              tags$div(
                class = "workflow-action-row workflow-split-row",
                actionButton("go_convert_from_overview", tagList(icon("shuffle"), " 去 空间截面数据转换"), class = "btn btn-outline-success run-btn"),
            ...
            bs4Card(
              ...
              fluidRow(
                column(
                  ...
                  tags$div(
                    ...
                    tags$p(tags$strong("宋超"), "：项目负责人、统计理论指导、版权持有人。"),
                    tags$p(tags$strong("唐先腾"), "：项目开发人，负责系统设计、程序实现与使用支持。")
                  )
                ),
                column(
                  6,
              ...
              fluidRow(
                column(
                  ...
                  tags$div(
                    class = "info-block",
                    tags$strong(icon("home"), " 个人主页"),
                    tags$p(tags$strong("宋超"), "：", tags$a(href = "https://chaosong.blog/", target = "_blank", "https://chaosong.blog/")),
                    tags$p(tags$strong("唐先腾"), "：", tags$a(href = "https://tangxxxxt.github.io/", target = "_blank", "https://tangxxxxt.github.io/"))
                  )
                ),
                column(
                  6,
                  tags$div(
                    ...
                    tags$p("感谢四川大学的", tags$strong("薛锦"), "、", tags$strong("杨紫竹"), "、", tags$strong("雷蒙蒙"), "、", tags$strong("蒋蔺博"), "硕士以及西南石油大学的", tags$strong("唐章英"), "老师在技术及使用上对本交互式建模系统的全力支持。")
                  )
                )
              ),
              tags$div(
                class = "info-block",
                tags$strong(icon("quote-left"), " 建议引用"),
                tags$ul(
                  class = "qa-list",
                  tags$li(
              ...
              tags$div(
                ...
                tags$ul(
                  class = "qa-list",
                  tags$li("全局配置：Shiny 默认上传大小偏小，空间面板数据和 shapefile 经常会超过默认限制，因此本系统已先把上传上限调整为 1GB。"),
